...
ModbusRequestCallback modbusParseRequest05Callback=0;
...
{
	modbusParseRequest05Callback = cb;
}
uint8_t modbusParseRequest05( ModbusSlave *status, ModbusParser *parser )
{
	...
	if (modbusParseRequest05Callback)
		modbusParseRequest05Callback(parser->request05.index>>8);	
