...
void home_position(void)
{
	...
	float temp = med;
	factor_calc(0,0,joint_1,temp);
	factor_calc(0,0,joint_2,temp);
	factor_calc(0,0,joint_3,temp);
	factor_calc(0,360,joint_4,temp);
  write_pulse_4motor(temp,15,100);
...
void servo_move_home(void)
{
	...
	factor_calc(0,360,joint_1,home_time);
	factor_calc(0,150,joint_2,home_time);
	factor_calc(0,0,joint_3,home_time);
  factor_calc(0,0,joint_4,home_time);
	float error =0;
	int k = 0;
	
	write_pulse_4motor(home_time,15,200);
...
void get_position(float xf, float yf,float phi, float tf)
{
	...
	// Tinh cac he so cho 4 khop
	factor_calc(last_theta[SERVO1],new_theta[SERVO1],joint_1,tf);
	factor_calc(last_theta[SERVO2],new_theta[SERVO2],joint_2,tf);
	factor_calc(last_theta[SERVO3],new_theta[SERVO3],joint_3,tf);
	factor_calc(last_theta[SERVO4],new_theta[SERVO4],joint_4,tf);
...
void test_motor(float tf)
{
	...
	for (int step = 1;step <=(tf*1000);step++)
	{
		pulse_total(step);
		...
		if (step%400 == 0)
		{
			for (int servo = 0;servo< MAX_SERVO; servo++)
			{
				...
				if(servo == SERVO3)
				{
					//fix_pulse[servo] = fix_pulse_calc(error,10000.0,dir[servo]);
				}
				else {}
			}
		}
